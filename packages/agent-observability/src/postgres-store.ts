...
type RunEventRow = {
  event_id: string;
  run_id: string;
  event_type: "state" | "tool_call" | "log";
...
function mapWorkflowQueueJob(row: WorkflowQueueJobRow): WorkflowQueueJob {
  return {
    id: row.id,
    runId: row.run_id,
    agentId: row.agent_id,
...
export class PostgresObservabilityStore implements ObservabilityStore {
  ...
  async read(): Promise<DashboardData> {
  ...
  private async listRecentEvents(limit: number): Promise<RunEvent[]> {
    const result = await this.pool.query<RunEventRow>(
      `SELECT event_id, run_id, event_type, level, message, payload, occurred_at,
  ...
  async listRuns(filter?: RunsFilter): Promise<Run[]> {
    ...
    const result = await this.pool.query<RunRow>(
      `SELECT id, agent_id, status, started_at, ended_at, latency_ms, error_summary, trace_id, retries
         FROM runs
         ${where}
  ...
  async upsertRun(run: Run): Promise<void> {
    await this.pool.query(
      `INSERT INTO runs (id, agent_id, status, started_at, ended_at, latency_ms, error_summary, trace_id, retries)
  ...
  async appendRunEvent(runEvent: RunEvent): Promise<void> {
    const client = await this.pool.connect();
    try {
      await client.query("BEGIN");
      await client.query("SELECT pg_advisory_xact_lock(hashtext($1));", [runEvent.runId]);

      const nextStreamPosition = await client.query<{ position: string }>(
        `SELECT (COALESCE(MAX(stream_position), 0) + 1)::bigint AS position
           FROM run_events
           WHERE run_id = $1`,
        [runEvent.runId]
      );
  ...
  async enqueueWorkflowJob(input: WorkflowQueueJobCreateInput): Promise<WorkflowQueueJob> {
  ...
  async claimWorkflowJobs(input: ClaimWorkflowJobsInput): Promise<WorkflowQueueJob[]> {
    ...
    try {
      ...
      const claimed: WorkflowQueueJob[] = [];
      for (const row of candidateRows.rows) {
        const leaseToken = `${input.workerId}:${uuidv7()}`;
        const updated = await client.query<WorkflowQueueJobRow>(
          `UPDATE workflow_queue_jobs
            ...
            RETURNING
              ...
              objective_prompt, status, attempt_count, max_attempts, available_at, lease_token,
              lease_expires_at, last_error, created_at, updated_at`,
          [now, leaseToken, input.leaseMs, row.id]
        );
        if (updated.rowCount) {
  ...
  async completeWorkflowJob(input: CompleteWorkflowJobInput): Promise<void> {
    await this.pool.query(
      `UPDATE workflow_queue_jobs
  ...
  async failWorkflowJob(input: FailWorkflowJobInput): Promise<void> {
    ...
    await this.pool.query(
      `UPDATE workflow_queue_jobs
        ...
        WHERE id = $1
          AND lease_token = $2`,
      [input.jobId, input.leaseToken, input.error, retryAt]
    );
  }

  async getWorkflowJob(jobId: string): Promise<WorkflowQueueJob | undefined> {
    const result = await this.pool.query<WorkflowQueueJobRow>(
      `SELECT
          id, run_id, agent_id, tenant_id, workspace_id, workflow_id, request_id, thread_id,
          objective_prompt, status, attempt_count, max_attempts, available_at, lease_token,
