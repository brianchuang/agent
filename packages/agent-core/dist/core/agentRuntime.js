...
exports.AgentRuntime = exports.EnvelopeValidationError = void 0;
...
class AgentRuntime {
    ...
    telemetry;
    persistence;
    constructor(workspace, memory, telemetryConfig, persistence) {
        this.workspace = workspace;
        this.memory = memory;
        this.telemetry = new observability_1.RuntimeTelemetry(workspace, telemetryConfig);
        this.persistence = persistence ?? new repositories_1.InMemoryAgentPersistence();
    }
    ...
    async run(req) {
        if (!this.memory) {
            throw new errors_1.InternalRuntimeError("Memory engine is required for objective runtime execution");
        }
        ...
        const runId = `run_${(0, uuidv7_1.uuidv7)()}`;
        const traceId = `tr_${(0, uuidv7_1.uuidv7)()}`;
        ...
        try {
            ...
            if (validationIssues.length > 0) {
                throw new validation_1.PayloadValidationError(objective.id, event.type, validationIssues);
            }
    ...
    async runPlannerIntentStep(request, intent, deps) {
        ...
        try {
            ...
            const toolResult = deps?.toolRegistry
                ...
                : deps?.executeTool
                    ...
                    : (() => {
                        throw new errors_1.ValidationRuntimeError(`No tool executor configured for planner tool_call: ${intent.toolName}`);
                    })();
    ...
    async runPlannerLoop(request, deps) {
        this.validateObjectiveRequest(request);
        if (!deps) {
            throw new errors_1.ValidationRuntimeError("Invalid planner loop dependency: deps are required");
        }
        if (!deps.stages?.plan && typeof deps.planner !== "function") {
            throw new errors_1.ValidationRuntimeError("Invalid planner loop dependency: planner is required when plan stage is not provided");
        }
        const maxSteps = deps.maxSteps ?? 32;
        if (!Number.isInteger(maxSteps) || maxSteps <= 0) {
            throw new errors_1.ValidationRuntimeError("Invalid max step guard: maxSteps must be a positive integer");
        }
        ...
        if (workflow.status === "waiting_signal") {
            throw new errors_1.SignalValidationError(`Workflow is waiting for signal before next step: ${workflow.workflowId}`);
        }
        ...
        while (true) {
            ...
            if (workflow.pendingApproval?.status === "approved") {
                ...
                try {
                    const transition = await this.persistence.withTransaction(async (tx) => {
                        const currentWorkflow = tx.getWorkflow({
                            tenantId: request.tenantId,
                            workspaceId: request.workspaceId,
                            workflowId: request.workflowId
                        }) ??
            ...
            if (workflow.steps.length >= maxSteps) {
                workflow.status = "failed";
                throw new errors_1.ValidationRuntimeError(`Workflow exceeded max step guard (${maxSteps})`);
            }
            ...
            try {
                const transition = await this.persistence.withTransaction(async (tx) => {
                    ...
                    tx.appendAuditRecord({
                        ...
                        requestId: request.requestId,
                        stepNumber: stepIndex,
                        eventType: policyDecision.outcome === "allow"
                            ? "policy_allow"
                    ...
                    if (policyDecision.outcome === "block") {
                        ...
                        return {
                            workflow: currentWorkflow,
                            blockedError: new errors_1.PolicyBlockedError(policyDecision.policyId, `Policy blocked action (${policyDecision.reasonCode})`)
                        };
                    }
                ...
                await this.telemetry.onPlannerValidationFailure({
                    ...
                    occurredAt: request.occurredAt,
                    stepIndex,
                    phase: err instanceof errors_1.ValidationRuntimeError &&
    ...
    async resumeWithSignal(signal) {
        ...
        try {
            await this.persistence.withTransaction((tx) => {
                ...
                if (!checkpoint) {
                    throw new errors_1.SignalValidationError(`Workflow not found for resume: ${signal.workflowId}`);
                }
                ...
                if (!workflow) {
                    throw new errors_1.SignalValidationError(`Workflow not found for tenant/workspace: ${signal.workflowId}`);
                }
                ...
                if (signal.type === "approval_signal") {
                    const approvalPayload = signal.payload;
                    const pendingApproval = workflow.pendingApproval;
    ...
    async resumeWithProviderCallback(callback) {
    ...
    listAuditRecords(query) {
        if (!query.tenantId) {
            throw new errors_1.ValidationRuntimeError("Invalid audit query: tenantId is required");
        }
        if (!query.workspaceId) {
            throw new errors_1.ValidationRuntimeError("Invalid audit query: workspaceId is required");
        }
        if (query.workspaceId !== this.workspace) {
            throw new errors_1.ValidationRuntimeError(`Workspace mismatch: runtime=${this.workspace} query=${query.workspaceId}`);
        }
        return this.persistence.listAuditRecords(query);
    }
    validateObjectiveRequest(request) {
        if (request.schemaVersion !== "v1") {
            throw new errors_1.ValidationRuntimeError(`Unsupported objective request schemaVersion: ${String(request.schemaVersion)}`);
        }
        if (!UUID_V7_RE.test(request.requestId)) {
            throw new errors_1.ValidationRuntimeError("Invalid requestId: expected UUID v7");
        }
        if (!request.tenantId) {
            throw new errors_1.ValidationRuntimeError("Invalid tenantId: value is required");
        }
        if (!request.workspaceId) {
            throw new errors_1.ValidationRuntimeError("Invalid workspaceId: value is required");
        }
        if (request.workspaceId !== this.workspace) {
            throw new errors_1.ValidationRuntimeError(`Workspace mismatch: runtime=${this.workspace} request=${request.workspaceId}`);
        }
        if (!request.workflowId || !request.threadId) {
            throw new errors_1.ValidationRuntimeError("Invalid workflow identity: workflowId and threadId are required");
        }
        if (!request.objective_prompt || typeof request.objective_prompt !== "string") {
            throw new errors_1.ValidationRuntimeError("Invalid objective_prompt: non-empty string required");
        }
        this.assertIsoDatetime(request.occurredAt, "occurredAt");
    }
    validatePlannerIntent(intent) {
        if (intent.type === "tool_call") {
            if (!intent.toolName || typeof intent.toolName !== "string") {
                throw new errors_1.ValidationRuntimeError("Invalid planner intent: toolName is required for tool_call");
            }
            if (!this.isRecord(intent.args)) {
                throw new errors_1.ValidationRuntimeError("Invalid planner intent: args must be an object");
            }
            return;
        }
        if (intent.type === "ask_user") {
            if (!intent.question || typeof intent.question !== "string") {
                throw new errors_1.ValidationRuntimeError("Invalid planner intent: question is required for ask_user");
            }
            return;
        }
        if (intent.type === "complete") {
            if (intent.output !== undefined && !this.isRecord(intent.output)) {
                throw new errors_1.ValidationRuntimeError("Invalid planner intent: output must be an object when provided");
            }
            return;
        }
        throw new errors_1.ValidationRuntimeError(`Invalid planner intent type: ${intent.type}`);
    }
    validatePolicyDecision(decision, intent) {
        if (!decision.policyId || typeof decision.policyId !== "string") {
            throw new errors_1.ValidationRuntimeError("Invalid policy decision: policyId is required");
        }
        if (decision.outcome !== "allow" &&
            decision.outcome !== "block" &&
            decision.outcome !== "rewrite") {
            throw new errors_1.ValidationRuntimeError(`Invalid policy decision outcome: ${String(decision.outcome)}`);
        }
        if (!decision.reasonCode || typeof decision.reasonCode !== "string") {
            throw new errors_1.ValidationRuntimeError("Invalid policy decision: reasonCode is required");
        }
        if (decision.outcome === "rewrite") {
            if (!decision.rewrittenIntent) {
                throw new errors_1.ValidationRuntimeError("Invalid policy decision: rewrittenIntent is required for rewrite outcome");
            }
            this.validatePlannerIntent(decision.rewrittenIntent);
            return;
        }
        if (decision.rewrittenIntent) {
            throw new errors_1.ValidationRuntimeError("Invalid policy decision: rewrittenIntent is only allowed for rewrite outcome");
        }
        this.validatePlannerIntent(intent);
    }
    validateApprovalRequirement(requirement) {
        if (!requirement.riskClass || typeof requirement.riskClass !== "string") {
            throw new errors_1.ValidationRuntimeError("Invalid approval requirement: riskClass is required");
        }
        if (typeof requirement.requiresApproval !== "boolean") {
            throw new errors_1.ValidationRuntimeError("Invalid approval requirement: requiresApproval must be boolean");
        }
        if (!requirement.reasonCode || typeof requirement.reasonCode !== "string") {
            throw new errors_1.ValidationRuntimeError("Invalid approval requirement: reasonCode is required");
        }
    }
    defaultBuildPlanningContext = ({ request, stepIndex, priorSteps, toolRegistry, contextProvider }) => {
    ...
    defaultPlanStage = async (input, deps) => {
        if (typeof deps.planner !== "function") {
            throw new errors_1.ValidationRuntimeError("Invalid planner loop dependency: planner is required");
        }
        return await deps.planner(input);
    };
    defaultEvaluatePolicyStage = async ({ request, stepIndex, intent, plannerInput, policyEngine, policyPackResolver }) => {
        const policyPack = (await policyPackResolver?.({ request, stepIndex })) ?? {
        ...
        if (!policyPack.policyPackId || !policyPack.policyPackVersion) {
            throw new errors_1.ValidationRuntimeError("Invalid policy pack scope: policyPackId and policyPackVersion are required");
        }
        if (!policyEngine) {
            return {
    ...
    defaultEvaluateApprovalStage = async ({ request, stepIndex, intent, plannerInput, approvalPolicy }) => {
        if (intent.type !== "tool_call") {
            return {
                riskClass: "low",
    ...
    defaultExecuteIntentStage = async ({ request, stepIndex, intent, executeTool, toolRegistry }) => {
    ...
    validateSignal(signal) {
        if (signal.schemaVersion !== "v1") {
            throw new errors_1.SignalValidationError(`Unsupported signal schemaVersion: ${String(signal.schemaVersion)}`);
        }
        if (!UUID_V7_RE.test(signal.signalId)) {
            throw new errors_1.SignalValidationError("Invalid signalId: expected UUID v7");
        }
        if (!signal.tenantId || !signal.workspaceId || !signal.workflowId) {
            throw new errors_1.SignalValidationError("Invalid signal identity: tenantId, workspaceId, and workflowId are required");
        }
        this.assertIsoDatetime(signal.occurredAt, "occurredAt");
        if (signal.type === "approval_signal") {
            if (!this.isRecord(signal.payload)) {
                throw new errors_1.SignalValidationError("Invalid approval_signal payload: payload must be an object");
            }
            const approved = signal.payload.approved;
            const approverId = signal.payload.approverId;
            if (typeof approved !== "boolean") {
                throw new errors_1.SignalValidationError("Invalid approval_signal payload: approved must be boolean");
            }
            if (!approverId || typeof approverId !== "string") {
                throw new errors_1.SignalValidationError("Invalid approval_signal payload: approverId is required");
            }
            return;
        }
        if (signal.type === "external_event_signal") {
            if (!this.isRecord(signal.payload) || typeof signal.payload.eventType !== "string") {
                throw new errors_1.SignalValidationError("Invalid external_event_signal payload: eventType is required");
            }
            return;
        }
        if (signal.type === "timer_signal") {
            if (!this.isRecord(signal.payload) || typeof signal.payload.firedAt !== "string") {
                throw new errors_1.SignalValidationError("Invalid timer_signal payload: firedAt is required");
            }
        ...
        if (signal.type === "user_input_signal") {
            if (!this.isRecord(signal.payload) || typeof signal.payload.message !== "string") {
                throw new errors_1.SignalValidationError("Invalid user_input_signal payload: message is required");
            }
            return;
        }
        throw new errors_1.SignalValidationError(`Unsupported signal type: ${signal.type}`);
    }
    validateProviderCallback(callback) {
        if (callback.schemaVersion !== "v1") {
            throw new errors_1.SignalValidationError(`Unsupported callback schemaVersion: ${String(callback.schemaVersion)}`);
        }
        if (!UUID_V7_RE.test(callback.callbackId)) {
            throw new errors_1.SignalValidationError("Invalid callbackId: expected UUID v7");
        }
        if (!callback.tenantId || !callback.workspaceId || !callback.workflowId) {
            throw new errors_1.SignalValidationError("Invalid callback identity: tenantId, workspaceId, and workflowId are required");
        }
        if (!callback.eventType || typeof callback.eventType !== "string") {
            throw new errors_1.SignalValidationError("Invalid callback payload: eventType is required");
        }
        if (!this.isRecord(callback.payload)) {
            throw new errors_1.SignalValidationError("Invalid callback payload: payload must be an object");
        }
        this.assertIsoDatetime(callback.occurredAt, "occurredAt");
    }
    rethrowTypedError(err) {
        if (err instanceof errors_1.ValidationRuntimeError) {
            throw err;
        }
        if (err instanceof errors_1.PolicyBlockedError) {
            throw err;
        }
        if (err instanceof errors_1.ApprovalRequiredError) {
            throw err;
        }
        if (err instanceof errors_1.ToolExecutionError) {
            throw err;
        }
        if (err instanceof errors_1.RuntimeError) {
            throw err;
        }
        const message = err instanceof Error ? err.message : String(err);
        throw new errors_1.InternalRuntimeError(`Unhandled runtime error: ${message}`);
    }
    assertWorkflowScope(request, workflow) {
        if (workflow.tenantId !== request.tenantId ||
            ...
            throw new errors_1.SignalValidationError(`Workflow not found for tenant/workspace/thread: ${workflow.workflowId}`);
        }
    }
    toPlannerLoopResult(workflow) {
        return {
    ...
    assertIsoDatetime(value, field) {
        ...
        if (Number.isNaN(occurred.getTime()) || occurred.toISOString() !== value) {
            throw new errors_1.ValidationRuntimeError(`Invalid ${field}: expected ISO datetime`);
        }
    }
    isRecord(input) {
