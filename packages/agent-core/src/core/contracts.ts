...
export interface PlannerContextProvider {
  memory?: (input: { request: ObjectiveRequestV1; stepIndex: number }) => Record<string, unknown>;
  policyConstraints?: (input: { request: ObjectiveRequestV1; stepIndex: number }) => string[];
}
...
export type PolicyPackResolver = (
  input: PolicyPackResolverInput
) => PolicyPackReference | Promise<PolicyPackReference>;
...
export interface PlannerLoopStages {
  ...
  evaluateApproval: (
    input: PlannerApprovalStageInput
  ) => ApprovalRequirement | Promise<ApprovalRequirement>;
