export const REQUIRED_AGENT_TABLES = [
  "objective_requests",
  "workflow_instances",
  "planner_steps",
  "workflow_signals",
...
export const AGENT_SCHEMA_RETENTION: Record<RequiredAgentTable, RetentionPolicy> = {
  objective_requests: {
...
function requiredTenantColumns(): ColumnDefinition[] {
  return [
...
function buildInitialTables(): TableDefinition[] {
  return [
    {
...
function buildInitialIndexes(): IndexDefinition[] {
  return [
    {
...
export const AGENT_SCHEMA_MIGRATIONS: SchemaMigration[] = [
  {
    ...
    operations: [
      ...buildInitialTables().map((table) => ({ kind: "create_table", table }) as const),
      ...buildInitialIndexes().map((index) => ({ kind: "create_index", index }) as const)
    ]
  }
];

export function createEmptySchemaCatalog(): SchemaCatalog {
  return {
    tables: new Map<string, CatalogTable>(),
    indexes: new Map<string, IndexDefinition>(),
    appliedMigrations: []
  };
}

export function applyMigration(catalog: SchemaCatalog, migration: SchemaMigration): void {
...
export function createFreshAgentSchemaCatalog(): SchemaCatalog {
  const catalog = createEmptySchemaCatalog();
  for (const migration of AGENT_SCHEMA_MIGRATIONS) {
    applyMigration(catalog, migration);
  }
  return catalog;
}

const REPLAY_CRITICAL_INDEXES: Array<{ name: string; columns: string[] }> = [
  {
    name: "idx_objective_requests_workflow",
    columns: ["tenant_id", "workspace_id", "workflow_id"]
  },
...
export function validateReplayCriticalIndexes(catalog: SchemaCatalog): string[] {
  ...
  for (const requiredIndex of REPLAY_CRITICAL_INDEXES) {
    ...
    const sameColumns =
      found.columns.length === requiredIndex.columns.length &&
