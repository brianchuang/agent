...
export class RuntimeTelemetry {
  ...
  constructor(
    ...
    this.store = config.store ?? getObservabilityStore();
  }
  ...
  private workflowRunId(input: { tenantId: string; workspaceId: string; workflowId: string }) {
  ...
  private async ensureWorkflowRun(input: PlannerMetricBase & { status: RunStatus }) {
  ...
  async onRunQueued(params: { runId: string; traceId: string; startedAt: string }) {
    if (!this.isEnabled()) {
      return;
    }
  ...
  async onRunFinished(params: {
    ...
    const avgLatencyMs = latencyRuns.length
      ? Math.round(
          latencyRuns.reduce((acc, run) => acc + (run.latencyMs ?? 0), 0) / latencyRuns.length
        )
      : latencyMs;
  ...
  async onPlannerRequestReceived(input: PlannerMetricBase & { objectivePrompt: string }) {
    if (!this.isEnabled()) {
      return;
    }
  ...
  async onPlannerStepLatency(
    input: PlannerMetricBase & { stepIndex: number; latencyMs: number; status: string; intentType: string }
  ) {
    if (!this.isEnabled()) {
      return;
    }
  ...
  async onPlannerValidationFailure(
    input: PlannerMetricBase & { phase: string; errorMessage: string; stepIndex: number }
  ) {
    if (!this.isEnabled()) {
      return;
    }
  ...
  async onPolicyDecision(
    input: PlannerMetricBase & {
      stepIndex: number;
  ...
  async onWorkflowTerminal(input: PlannerMetricBase & { status: "completed" | "failed"; errorSummary?: string }) {
    if (!this.isEnabled()) {
      return;
    }
    ...
    const latencyMs = Math.max(
      1,
      new Date(input.occurredAt).getTime() - new Date(startedAt).getTime()
    );
