...
const TIER_PRIORITY = { canonical: 3, distilled: 2, raw: 1 } as const;
...
export class MemoryEngine {
  ...
  private readonly workingMemory = new Map<string, WorkingMemorySnapshot>();
  ...
  retrieve(query: RetrievalQuery, budget: RetrievalBudget): RetrievalResult {
    ...
    const gated = this.items.filter((item) => {
      ...
      if (item.metadata.workspace !== query.workspace) return false;
      if (item.metadata.objective !== query.objective) return false;
      if (query.channel && item.metadata.channel !== query.channel) return false;
      if (query.language && item.metadata.language !== query.language) return false;
      if (query.accountTier && item.metadata.accountTier !== query.accountTier) return false;
      if (minDate && item.createdAt < minDate && item.tier !== "canonical") return false;
    ...
    for (const candidate of scored) {
      ...
      if (picked.length >= budget.maxItems) break;

      const categoryLimit = budget.maxByCategory[item.category];
      const usedInCategory = categoryCounts[item.category] ?? 0;
      if (categoryLimit !== undefined && usedInCategory >= categoryLimit) continue;

      const duplicate = picked.some(
        (existing) => jaccardSimilarity(existing.summary, item.summary) > 0.9
      );
      if (duplicate) continue;

      const conflict = picked.find(
        (existing) =>
          existing.contradictionGroup &&
      ...
      if (tokens + itemTokens > budget.maxTokens) continue;
  ...
  promoteRawToDistilled(reasonTag: string): MemoryItem | undefined {
    const batch = this.items.filter(
      (x) => x.tier === "raw" && !x.archivedAt && x.metadata.tags.includes(reasonTag)
    );
  ...
  markCanonical(itemId: string, approvedByHuman = true): MemoryItem | undefined {
    const item = this.items.find((x) => x.id === itemId);
  ...
  applyDecayAndArchive(maxAgeDays = 90, minUseBeforeKeep = 2): number {
    const now = nowUtc();
    let archived = 0;
