...
import { EventEnvelopeV1 } from "./core/contracts";
import { MemoryEngine } from "./memory";
import { InterviewObjectivePlugin } from "./objectives/interview/objective";
...
async function main(): Promise<void> {
  ...
  const candidate = reg.result.output?.candidate as { id: string };

  const sch = await runtime.run(
    envelope("interview.schedule", candidate.id, {
  ...
  const interview = sch.result.output?.interview as { id: string };

  const draft = await runtime.run(
    envelope("message.draft", candidate.id, {
      candidateId: candidate.id,
  ...
  await runtime.run(
    envelope("interview.complete", candidate.id, {
      interviewId: interview.id,
      feedback:
        "Strong coding fundamentals, clear communication, mild concerns around distributed systems depth."
    })
  );
  ...
  const actions = await runtime.run(
    envelope("actions.suggest", candidate.id, {
      candidateId: candidate.id
    })
  );
  const nextActions = (actions.result.output?.actions as string[]) ?? [];
